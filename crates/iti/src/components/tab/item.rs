...
//! own `StepWithMut` closure receives `&mut TabPanelEntry` and can delegate
...
pub struct TabListItem<V: View, T = <V as View>::Element> {
    #[child]
    #[properties]
    li: V::Element,
    ...
    id: Id<T>,
    // Index cache - care must be taken to ensure this is correct
    index: usize,
}

impl<V: View, T: ViewChild<V> + 'static> TabListItem<V, T> {
    ...
    /// without a lookup.
    pub fn new(id: Id<T>, index: usize, inner: T) -> Self {
...
pub struct TabListItemEventData<V: View, T = <V as View>::Element> {
    pub id: Id<T>,
    pub index: usize,
    pub event: V::Event,
}
...
pub enum TabListItemEvent<V: View, T = <V as View>::Element, Ev = ()> {
    Click(TabListItemEventData<V, T>),
    Close(TabListItemEventData<V, T>),
    User(Ev),
}

/// Race a tab's click and close-button click listeners, returning the first
/// to fire as a [`TabListItemEvent`].
async fn step_impl<V: View, T, Ev: 'static>(
    on_click: &V::EventListener,
    on_close: &V::EventListener,
    id: Id<T>,
    index: usize,
) -> TabListItemEvent<V, T, Ev> {
...
impl<V: View, T: 'static> Step for TabListItem<V, T> {
    type Output = TabListItemEvent<V, T>;
...
impl<V: View, T: 'static> StepWith<T> for TabListItem<V, T> {
    type Output<Ev: 'static> = TabListItemEvent<V, T, Ev>;
...
impl<V: View, T: 'static> StepWithMut<T> for TabListItem<V, T> {
    type Output<Ev: 'static> = TabListItemEvent<V, T, Ev>;
